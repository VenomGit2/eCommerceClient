...
export default function WishlistPage() {
  ...
  return <section className="account-page"><AccountTabs /><div className="account-page__heading"><p className="eyebrow">Your account</p><h1>Wishlist</h1></div>{items.length ? <div className="product-grid">{items.map((item, index) => <ProductCard key={item.id ?? index} product={item} onAdd={addItem} isInCart={cartItems.some((cartItem) => String(cartItem.id) === String(item.id))} />)}</div> : <EmptyState title="Your wishlist is empty" message="Saved products will appear here." />}</section>;
}
