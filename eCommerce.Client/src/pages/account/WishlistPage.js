...
export default function WishlistPage() {
  const { session } = useAuth(); const { addItem, items: cartItems } = useCart(); const result = useAsync((signal) => getWishlist(session?.token, signal), [session?.token]); const items = getCollection(result.data);
