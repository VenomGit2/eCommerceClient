...
export default function UpdateProductPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      await updateProduct(productId.trim(), { ...details, unitPrice: Number(details.unitPrice), quantityInStock: Number(details.quantityInStock) });
  ...
  return <section><p className="eyebrow">Administration</p><h1>Update product</h1><AdminProductTabs />{state.error && <ErrorMessage message={state.error} />}{state.success && <p className="notice notice--success" role="status">{state.success}</p>}<form className="admin-product-form" onSubmit={submit}><ProductFormFields product={product} setProduct={setProduct} includeProductId /><Button type="submit" disabled={state.submitting}>{state.submitting ? 'Updating product…' : 'Update product'}</Button></form></section>;
}
