  ...
  return <>
    ...
    <header className="site-header">
      ...
      <nav className="nav page-container" aria-label="Main navigation">
        <button className="nav__toggle" type="button" aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} aria-controls="mobile-navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}><span /><span /><span /></button>
        <div id="mobile-navigation" className={`nav__links nav__links--primary ${menuOpen ? 'nav__links--open' : ''}`}>
